...
ifeq ($(TARGET_OS),osx)
	TARGET_CFLAGS   = -mmacosx-version-min=10.7 -arch $(shell uname -m)
	TARGET_CXXFLAGS = -mmacosx-version-min=10.7 -arch $(shell uname -m) -stdlib=libc++
	TARGET_LDFLAGS  = -mmacosx-version-min=10.7 -arch $(shell uname -m) -stdlib=libc++
endif
